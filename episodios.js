 ...
 async function traerEpisodio(desdeId = 1) {
  ...
  if (!aside) return;
  aside.innerHTML = '<h2 class="text-2xl text-green-400 font-bold mb-8">Episodios</h2>';
...
async function mostrarPersonajesEpisodio(episodio) {
  ...
  if (!episodio.characters || episodio.characters.length === 0) {
    personajesContainer.innerHTML = '<p class="text-gray-500">No hay personajes para este episodio.</p>';
    return;
  }
  ...
  try {
    ...
    personajes.forEach(personaje => {
      ...
      cardPers.className = 'bg-white/90 rounded-lg shadow-lg p-3 flex flex-col items-center w-40 hover:scale-105 transition';
      cardPers.innerHTML = `
